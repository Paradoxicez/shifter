...
func TestRunMigrations_Clean(t *testing.T) {
	...
	// Each Phase 1 table must exist after a clean run.
	...
	// schema_migrations must be at version 6 (the highest migration), not dirty.
	var version int
	var dirty bool
	err = pool.QueryRow(ctx, `SELECT version, dirty FROM schema_migrations`).Scan(&version, &dirty)
	require.NoError(t, err)
	require.Equal(t, 6, version, "expected schema_migrations.version = 6")
...
func TestRunMigrations_Idempotent(t *testing.T) {
	...
	// Sanity: version is still 6 after the no-op.
	var version int
