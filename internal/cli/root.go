...
// (serve, migrate, version, create-admin, config-check, healthcheck) are
// registered in Execute(). D-12.
...
//
// The 6 canonical subcommands per D-12: serve, migrate, version, create-admin,
// config-check, healthcheck.
func Execute() error {
	rootCmd.AddCommand(
		serveCmd,
		migrateCmd,
