...
**Update-pass scope cut (2026-05-12):** Vendor catalog ships **4 entries / 3 codec files** (Axioma W1 + Acrel ADL200 + Acrel ADW300 sharing one codec + Itron+KINMY) instead of the original "7 vendors." Kamstrup MULTICAL / Diehl / Sagemcom / Schneider IEM3xxx are **deferred to v1.1+** — added via D-20 PR workflow when a real customer install requires them. The catalog architecture is fully validated with 4 entries; adding more is no longer system work. See D-18 (REVISED) for details.
...
- **D-18:** **(REVISED 2026-05-12) Ship 4 catalog entries from 3 existing codec files in the Phase 7 binary.** Scope reduction from the original "7 vendors" wording (and from ROADMAP SC#1) — Phase 7 promotes the existing JS codec substrate into a catalog and validates the end-to-end pipeline (catalog → import → ChirpStack sync → ingest → canonical) with **real codecs that operators are actually running**. Adding Kamstrup MULTICAL / Diehl / Sagemcom / Schneider is **deferred to v1.1+** via the established PR workflow (D-20) — those vendors come when a real customer install requires them, not speculatively. **Phase 7 ships these catalog entries:**
  - `axioma_w1` (Axioma Qalcosonic W1 — water — codec: `axioma_w1.js` from Phase 2) — promoted from "one wired profile" to "catalog entry with metadata"
  - `acrel_adl200` (Acrel ADL200 — electricity — codec: `acrel_family.js` from Phase 2, shared per D-32) — same promotion
  - `acrel_adw300` (Acrel ADW300 — electricity — codec: `acrel_family.js` from Phase 2, shared per D-32) — same promotion
  - `itron_kinmy_lora` (Itron Water Meter + KINMY LoRa Module — water — codec: `itron_kinmy_lora.js`, operator-supplied, adapted to Phase 2 convention 2026-05-12) — new

  That's **4 catalog rows, 3 codec source files** (Acrel shared). Each entry carries metadata: codec_js_path (D-19), capabilities, battery_curve (D-44), expected_uplink_interval_seconds (D-41), offline_threshold_multiplier (D-43), anomaly_compatibility (D-42), vendor_has_separate_meter_serial (D-45). **Rationale for scope cut:** (a) ship Phase 7 faster — no time spent authoring codecs we don't yet have customer demand for; (b) the catalog architecture is validated with 4 real entries — adding more later is a PR-workflow application, not a system change; (c) speculative codecs ship with unknown payload formats and untested fixtures — risk of shipping wrong defaults that operators then have to correct via D-04 customer-edit path. ROADMAP SC#1's "vendor #2 / #3 / #N are admin-UI work, not backend deploys" wording remains scope-revised for v1.x (see D-20).
...
- **Additional vendor catalog entries (Kamstrup MULTICAL, Diehl, Sagemcom, Schneider IEM3xxx, etc.)** — v1.1+ via D-20 PR workflow. **D-18 scope-revised 2026-05-12 to ship 4 entries (Axioma + Acrel ADL200 + Acrel ADW300 + Itron+KINMY) instead of the original 7.** Adding more is a per-vendor PR (codec.js + catalog.json + fixtures) → binary release — no architectural change. The catalog system itself is fully validated with 4 entries.
