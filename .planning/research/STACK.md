...
**Domain:** Self-hosted LoRaWAN water/electricity monitoring (ChirpStack-wrapping single-tenant web app)
**Researched:** 2026-04-27
**Confidence:** HIGH (Go backend choice, ChirpStack integration model, frontend stack); MEDIUM (specific Go library choices where idiomatic Go offers multiple equally good options)
...
**Pick Go.** Three things make this obvious for Shifter:

1. **gRPC interop with ChirpStack is native.** ChirpStack itself is written in Rust but its first-class client SDK is Go (`github.com/chirpstack/chirpstack/api/go/v4`, current v4.17.0+ as of March 2026). The same protobuf definitions ChirpStack publishes are auto-generated for Go and shipped as part of the ChirpStack monorepo — no third-party wrapper layer, no version drift. Python and JS clients exist (`chirpstack-api`, `@chirpstack/chirpstack-api`) but they're equivalent generated stubs, not advantaged over Go.
2. **Single-binary deploy is the operational model that fits "we deploy this on each customer's infrastructure."** A Go binary is 5–15MB, zero runtime dependencies, embeds the SPA via `go:embed`, and the entire customer install becomes "drop binary + write systemd unit + run docker-compose for Postgres+ChirpStack." Node/Python make this same install 50–80MB of `node_modules` or a Python venv with C-extension hazards (psycopg2, paho native bindings). Per-customer self-hosting punishes runtime complexity.
3. **Velocity is not the bottleneck Go skeptics think it is.** The slow-to-build Go reputation comes from web app framework gymnastics, but Shifter is mostly: gRPC client calls, MQTT subscriber loops, TimescaleDB inserts, REST endpoints for the SPA, scheduled aggregates, alert evaluation. Every one of these has best-in-class, boring Go tooling. FastAPI's velocity advantage shows in research/AI/ML scaffolding — not here.
...
**Confidence on backend language:** HIGH. The deciding factors (gRPC interop, single-binary deploy, ecosystem maturity for IoT-adjacent workloads) all point the same direction.
...
**Why Vite SPA, not Next.js:** Shifter is a logged-in dashboard. There is no SEO surface, no marketing site, no SSR/edge-rendering benefit. Next.js's value (RSC, ISR, dynamic OG images) is wasted; its cost (Node-server-at-runtime, more deploy surface, opinionated routing) is real. shadcn officially supports both, and the dashboard/admin-panel community has converged on Vite + React for self-hosted internal tools in 2026. *If* the product later needs a marketing site or public report-sharing pages, those can be a separate Next.js or static-site deploy.
...
**Use Server-Sent Events (SSE) from Shifter backend to the browser.** The data flow is:

```
ChirpStack ──MQTT──▶ Shifter (Go MQTT subscriber)
                       │
                       ├── INSERT into TimescaleDB hypertable
                       └── fan out to connected SSE subscribers (per-user, scoped to their visible meters)
```

**Why SSE, not WebSocket, not MQTT-over-WS:**
...
Use a **hybrid wide+JSONB pattern** in a single hypertable:

```sql
CREATE TABLE measurements (
    time         TIMESTAMPTZ        NOT NULL,
    metering_point_id  UUID         NOT NULL,
    -- Canonical fields the UI knows how to render (sparse columns):
    cumulative_value   DOUBLE PRECISION,   -- water m³, electricity kWh
    flow_rate          DOUBLE PRECISION,   -- l/min, instantaneous power, etc.
    battery_pct        SMALLINT,
    rssi               SMALLINT,
    snr                REAL,
    -- Everything else the device emitted:
    raw                JSONB              NOT NULL DEFAULT '{}'::jsonb
);
SELECT create_hypertable('measurements', 'time');
CREATE INDEX ON measurements (metering_point_id, time DESC);
CREATE INDEX ON measurements USING GIN (raw jsonb_path_ops);
```
...
**Two Compose files, single repo, single binary.**
...
The Shifter container is `FROM scratch` or `gcr.io/distroless/base` with the Go binary + embedded SPA — typically <30MB.
...
# Dev
pnpm add -D @biomejs/biome playwright
```
...
**If customer has existing ChirpStack:**
...
**If customer is greenfield (no LoRaWAN stack):**
...
**If customer monitors only water (or only electricity):**
...
**If customer fleet grows past ~50K devices:**
...
**ChirpStack (HIGH confidence — official):**
...
**Backend language comparison (MEDIUM — multiple corroborating sources):**
...
**Go libraries (HIGH — official package docs):**
...
**Frontend stack (HIGH — official + ecosystem-corroborated):**
...
*Stack research for: self-hosted LoRaWAN water/electricity monitoring (Shifter)*
*Researched: 2026-04-27*
